...
import { createHandler as createHfTransformersHandler } from "./providers/hf-transformers.js";
...
export const createChatSession = ({ provider, model, temperature }) => {
  ...
  const ensureHandler = async () => {
    ...
    } else if (provider === "hfTransformers") {
      state.handler = await createHfTransformersHandler({
        model,
        temperature,
